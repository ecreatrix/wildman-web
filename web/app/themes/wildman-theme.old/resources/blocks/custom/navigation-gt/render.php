...
namespace WA\Blocks\Navigation;
...
class Block_Renderer
{
    ...
    public static function render( $attributes, $content, $block )
    {
        ...
        if ( wa_block_core_navigation_block_contains_core_navigation( $inner_blocks ) ) {
            return '';
        }
    ...
    private static function does_block_need_a_list_item_wrapper( $block )
    {
         ...
         */
        $needs_list_item_wrapper = apply_filters( 'wa_block_core_navigation_listable_blocks', static::$needs_list_item_wrapper );
    ...
    private static function get_classes( $attributes )
    {
        ...
        $colors             = wa_block_core_navigation_build_css_colors( $attributes );
        $font_sizes         = wa_block_core_navigation_build_css_font_sizes( $attributes );
    ...
    private static function get_inner_blocks( $attributes, $block )
    {
        ...
        if (
            ...
            ! empty( wa_block_core_navigation_get_menu_items_at_location( $attributes['__unstableLocation'] ) )
        ) {
            $inner_blocks = wa_block_core_navigation_get_inner_blocks_from_unstable_location( $attributes );
        }
         ...
         */
        $inner_blocks = apply_filters( 'wa_block_core_navigation_render_inner_blocks', $inner_blocks );

        $post_ids = wa_block_core_navigation_get_post_ids( $inner_blocks );
        if ( $post_ids ) {
    ...
    {
        $fallback_blocks = wa_block_core_navigation_get_fallback_blocks();
    ...
    private static function get_inner_blocks_from_navigation_post( $attributes )
    {
        ...
        if ( 'publish' === $navigation_post->post_status ) {
            ...
            $blocks = wa_block_core_navigation_filter_out_empty_blocks( $parsed_blocks );
    ...
    private static function get_inner_blocks_html( $attributes, $inner_blocks )
    {
        ...
        $container_attributes = get_block_wrapper_attributes(
            [
                'class' => 'wa-block-navigation__container ' . $class,
                'style' => $style,
            ]
        );
        ...
        if ( $has_submenus && $is_interactive ) {
            ...
            $inner_blocks_html = wa_block_core_navigation_add_directives_to_submenu( $tags, $attributes );
        }
    ...
    private static function get_markup_for_inner_block( $inner_block )
    {
        ...
        if (  ! empty( $inner_block_content ) ) {
            if ( static::does_block_need_a_list_item_wrapper( $inner_block ) ) {
                return '<li class="wa-block-navigation-item">' . $inner_block_content . '</li>';
            }
        }
    ...
    private static function get_responsive_container_markup( $attributes, $inner_blocks, $inner_blocks_html )
    {
        ...
        $responsive_container_classes = [
            'wa-block-navigation__responsive-container',
            $is_hidden_by_default ? 'hidden-by-default' : '',
            implode( ' ', $colors['overlay_css_classes'] ),
        ];
        $open_button_classes = [
            'wa-block-navigation__responsive-container-open',
        ...
        return sprintf(
            '<button aria-haspopup="dialog" %3$s class="%6$s" %10$s>%8$s</button>
				<div class="%5$s" %7$s id="%1$s" %11$s>
					<div class="wa-block-navigation__responsive-close" tabindex="-1">
						<div class="wa-block-navigation__responsive-dialog" %12$s>
							<button %4$s class="wa-block-navigation__responsive-container-close" %13$s>%9$s</button>
							<div class="wa-block-navigation__responsive-container-content" %14$s id="%1$s-content">
								%2$s
							</div>
						</div>
					</div>
...
if ( defined( 'IS_GUTENBERG_PLUGIN' ) && IS_GUTENBERG_PLUGIN ) {
     ...
     */
    function wa_block_core_navigation_get_menu_items_at_location( $location )
    {
        if ( empty( $location ) ) {
            return;
        }
     ...
     */
    function wa_block_core_navigation_sort_menu_items_by_parent_id( $menu_items )
    {
        $sorted_menu_items = [];
     ...
     * @return WP_Block_List Returns the inner blocks for the navigation block.
     */
    function wa_block_core_navigation_get_inner_blocks_from_unstable_location( $attributes )
    {
        $menu_items = wa_block_core_navigation_get_menu_items_at_location( $attributes['__unstableLocation'] );
        if ( empty( $menu_items ) ) {
            return new WP_Block_List( [], $attributes );
        }

        $menu_items_by_parent_id = wa_block_core_navigation_sort_menu_items_by_parent_id( $menu_items );
        $parsed_blocks           = wa_block_core_navigation_parse_blocks_from_menu_items( $menu_items_by_parent_id[0], $menu_items_by_parent_id );
 ...
 */
function wa_block_core_navigation_add_directives_to_submenu( $tags, $block_attributes )
{
    while ( $tags->next_tag(
        [
            'tag_name'   => 'LI',
        ...
        if ( $tags->next_tag(
            [
                'tag_name'   => 'BUTTON',
                'class_name' => 'wa-block-navigation-submenu__toggle',
            ]
        ) ) {
        ...
        if ( $tags->next_tag(
            [
                'tag_name'   => 'UL',
                'class_name' => 'wa-block-navigation__submenu-container',
            ]
        ) ) {
        ...
        wa_block_core_navigation_add_directives_to_submenu( $tags, $block_attributes );
    }
 ...
 */
function wa_block_core_navigation_build_css_colors( $attributes )
{
    $colors = [
        'css_classes'           => [],
        'inline_styles'         => '',
 ...
 */
function wa_block_core_navigation_build_css_font_sizes( $attributes )
{
    // CSS classes.
    $font_sizes = [
        'css_classes'   => [],
 ...
 * @return string
 */
function wa_block_core_navigation_render_submenu_icon()
{
 ...
 */
function wa_block_core_navigation_filter_out_empty_blocks( $parsed_blocks )
{
 ...
 */
function wa_block_core_navigation_block_contains_core_navigation( $inner_blocks )
{
    foreach ( $inner_blocks as $block ) {
        if ( 'core/navigation' === $block->name ) {
            return true;
        }
        if ( $block->inner_blocks && wa_block_core_navigation_block_contains_core_navigation( $block->inner_blocks ) ) {
            return true;
        }
    }
 ...
 */
function wa_block_core_navigation_get_fallback_blocks()
{
    $page_list_fallback = [
        [
    ...
    if ( $navigation_post ) {
        $parsed_blocks  = parse_blocks( $navigation_post->post_content );
        $maybe_fallback = wa_block_core_navigation_filter_out_empty_blocks( $parsed_blocks );
     ...
     */
    return apply_filters( 'wa_block_core_navigation_render_fallback', $fallback_blocks );
}
 ...
 */
function wa_block_core_navigation_get_post_ids( $inner_blocks )
{
    $post_ids = array_map( 'wa_block_core_navigation_from_block_get_post_ids', iterator_to_array( $inner_blocks ) );
 ...
 * @return array Array of post IDs.
 */
function wa_block_core_navigation_from_block_get_post_ids( $block )
{
    $post_ids = [];

    if ( $block->inner_blocks ) {
        $post_ids = wa_block_core_navigation_get_post_ids( $block->inner_blocks );
    }
 ...
 * @return string Returns the navigation block markup.
 */
function wa_render_block_core_navigation( $attributes, $content, $block )
{
    return WA_Navigation_Block_Renderer::render( $attributes, $content, $block );
}
 ...
 */
function wa_block_core_navigation_typographic_presets_backcompatibility( $parsed_block )
{
...
add_filter( 'render_block_data', 'wa_block_core_navigation_typographic_presets_backcompatibility' );
 ...
 */
function wa_block_core_navigation_parse_blocks_from_menu_items( $menu_items, $menu_items_by_parent_id )
{
    ...
    foreach ( $menu_items as $menu_item ) {
        ...
        ? wa_block_core_navigation_parse_blocks_from_menu_items( $menu_items_by_parent_id[$menu_item->ID], $menu_items_by_parent_id )
        : [];
 ...
 */
function wa_block_core_navigation_get_classic_menu_fallback()
{
 ...
 * @return array the normalized parsed blocks.
 */
function wa_block_core_navigation_get_classic_menu_fallback_blocks( $classic_nav_menu )
{
    ...
    $inner_blocks = wa_block_core_navigation_parse_blocks_from_menu_items(
 ...
 * @return array the normalized parsed blocks.
 */
function wa_block_core_navigation_maybe_use_classic_menu_fallback()
{
    _deprecated_function( __FUNCTION__, '6.3.0', 'WP_Navigation_Fallback::create_classic_menu_fallback' );

    // See if we have a classic menu.
    $classic_nav_menu = wa_block_core_navigation_get_classic_menu_fallback();
    ...
    $classic_nav_menu_blocks = wa_block_core_navigation_get_classic_menu_fallback_blocks( $classic_nav_menu );
    ...
    return wa_block_core_navigation_get_most_recently_published_navigation();
}
 ...
 */
function wa_block_core_navigation_get_most_recently_published_navigation()
{
