 ...
 */
import useNavigationMenu from './utils/use-navigation-menu';
...
import NavigationInnerBlocks from './utils/inner-blocks';
import NavigationMenuNameControl from './utils/navigation-menu-name-control';
import UnsavedInnerBlocks from './utils/unsaved-inner-blocks';
import NavigationMenuDeleteControl from './utils/navigation-menu-delete-control';
import useNavigationNotice from './utils/use-navigation-notice';
import OverlayMenuIcon from './utils/overlay-menu-icon';
import OverlayMenuPreview from './utils/overlay-menu-preview';
import useConvertClassicToBlockMenu, {
	CLASSIC_MENU_CONVERSION_ERROR,
	CLASSIC_MENU_CONVERSION_PENDING,
	CLASSIC_MENU_CONVERSION_SUCCESS,
} from './utils/use-convert-classic-menu-to-block-menu';
import useCreateNavigationMenu from './utils/use-create-navigation-menu';
import { useInnerBlocks } from './utils/use-inner-blocks';
import { detectColors } from './utils/tools';
import ManageMenusButton from './utils/manage-menus-button';
import MenuInspectorControls from './utils/menu-inspector-controls';
import DeletedNavigationWarning from './utils/deleted-navigation-warning';
import AccessibleDescription from './utils/accessible-description';
import AccessibleMenuDescription from './utils/accessible-menu-description';
...
function Navigation( props ) {
	const {
		...
		// These props are used by the navigation editor to override specific navigation block settings.
	...
	// Preload classic menus, so that they don't suddenly pop-in when viewing the Select Menu dropdown.
	...
	//console.log(useNavigationMenu( ref ))
	...
	const overlayMenuPreviewClasses = clsx(
		'wa-block-navigation__overlay-menu-preview',
	...
	/**
	 * Historically the navigation block has supported custom placeholders. Even though the current UX tries as hard as possible not to end up in a placeholder state, the block continues to support this extensibility point, via a CustomPlaceholder. When CustomPlaceholder is present it becomes the default fallback for an empty navigation block, instead of the default fallbacks.
	 */
			...
			<RecursionProvider uniqueId={ recursionId }>
				...
				{ blockEditingMode === 'default' && isEntityAvailable && (
					<InspectorControls group="advanced">
						...
						<ManageMenusButton
							...
							className="wa-block-navigation-manage-menus-button"
						/>
					</InspectorControls>
				) }
				...
				>
					{ isLoading && ! isHiddenByDefault && (
						<div className="wa-block-navigation__loading-indicator-container">
							<Spinner className="wa-block-navigation__loading-indicator" />
						</div>
					) }

					{ ( ! isLoading && isNavigationMenuMissing ) && (
						<div icon={ icon }>Select menu from Inspector</div>
					) }

					{ ( ! isLoading || isHiddenByDefault ) && !isNavigationMenuMissing && (
						<>
