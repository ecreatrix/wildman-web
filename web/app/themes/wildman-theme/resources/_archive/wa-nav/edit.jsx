...
import { useEntityBlockEditor, EntityProvider, store as coreStore } from '@wordpress/core-data';
...
import { detectColors } from './utils/utils';
...
import { DEFAULT_BLOCK, PRIORITIZED_INSERTER_BLOCKS } from './utils/constants';
...
function Navigation( props ) {
	...
	const navBlocks = useSelect((select) => {
	...
	const blockContent = navBlocks && navBlocks.map( ( nav ) => {
		//console.log(nav);
	...
	const handleUpdateMenu = useCallback(
		...
		[ selectBlock, clientId, setRef, navBlocks ]
	);
	...
	const navigationFallbackId = ref || hasUnsavedBlocks || null;
	...
	const overlayMenuPreviewClasses = clsx(
		'wp-block-wa-nav__overlay-menu-preview',
	...
	const showPlaceholder =
		!! CustomPlaceholder && ! hasMenuItems && ! isSelected;

	const innerBlocksProps = useInnerBlocksProps(
		{
			className: 'wp-block-wa-header-mantine__container',
		},
		{
			value: blocks,
			onInput,
    ...
    const updateFieldValue = ( navBlocks ) => {
    	//console.log(navBlocks)
        setAttributes( { navItems: navBlocks } );
    }
		...
		{ blockEditingMode === 'default' && isEntityAvailable && (
			<InspectorControls group="advanced">
				...
				<ManageMenusButton
					...
					className="wp-block-wa-nav-manage-menus-button"
				/>
			</InspectorControls>
		) }
		<div { ...blockProps }
			...
			<RecursionProvider uniqueId={ recursionId }>
				...
				>
					{ isLoading && ! isHiddenByDefault && (
						<div className="wp-block-wa-nav__loading-indicator-container">
							<Spinner className="wp-block-wa-nav__loading-indicator" />
						</div>
					) }
